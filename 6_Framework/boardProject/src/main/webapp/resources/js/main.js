...
btn1.addEventListener("click", ()=>{
    ...
    .then(resp => resp.text())
    // resp : 응답 객체
    // resp.text : 응답 객체 내용을 문자열로 반환하여 반환
...
btn2.addEventListener("click", () => {
    ...
    fetch("/selectMember", {
        ...
        body : JSON.stringify({"email" : inputEmail.value})  // JS객체
    })
...
btn3.addEventListener("click", () => {
    ...
    fetch("/selectMemberList",{
        method : "POST",
        headers : {"Content-Type" : "application/text"}, // 문자열 하나를 파라미터로 전달 Map으로하는게 쉬워서 잘 안씀
    ...
    .then(memberList => {
        ...
        if(memberList.length == 0){    // for문 안돌리기위해 앞에서
