...
@SessionAttributes({"loginMember"}) // Model의 이름(key)값을 적으면 session으로 추가 (여러 개일 경우 배열 형태로 작성)
public class MemberController {
	...
	public String login(/*@ModelAttribute*/ Member inputMember) {
		...
		// - 전달받은 파라미터의 name 속성 값
		//	 같이 사용되는 DTO의 필드명과 같다면
		//	 자동으로 setter를 호출해 필드에 값을 세팅
	...
	public String signUp() {
		...
		// /WEB-INF/views/member/signUp.jsp로 forward(요청, 위임)
	...
	public String signUp(Member inputMember
		...
		ra.addFlashAttribute("message", message); // forward할 때는 redirectAttributes 대신 model 사용해서 addFlash 해도 되는지?!?!!?? 된대용
		
		
		return path;
	}
	
	/* Spring 예외 처리 방법 (3종류, 우선순위, 중복 사용)
	 * 
	 * 1순위 : 메소드 단위로 처리
	 * 	    -> try-catch / throws
	 * 
